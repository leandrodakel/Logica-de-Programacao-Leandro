			...
			{	
				...
				for(int j = 0; j < 1; j++)
				{
						for(int w = 0; w < tam_um; w++)
						{
							...
							soma_um += deposito[k].descricao[w];
						}
						for(int x = 0; x < tam_dois; x++)
						{
							printf("entrou no for soma2 \n");
							soma_dois += deposito[i].descricao[x];
						}
				}	
				printf("Soma um = %d\n",soma_um);
				printf("Soma dois = %d\n",soma_dois);
				...
				//validando troca de vetores
					if(deposito[k].descricao[cont] > deposito[i].descricao[cont])
					{
							...
							soma_um += deposito[k].descricao[cont];
							soma_dois += deposito[i].descricao[cont];
							troca = 1;
							printf("troca = 1\n");
							printf("necessario trocar %s com %s\n",deposito[k].descricao,deposito[i].descricao);
					...
					if(deposito[k].descricao[tam_menor - 1] == deposito[i].descricao[tam_menor - 1] && letra_igual == (tam_menor - 1))
					{
